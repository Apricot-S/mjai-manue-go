...
func TestEndGame_ToEvent(t *testing.T) {
	type fields struct {
		Message Message
	}
	tests := []struct {
		name   string
		fields fields
		want   *inbound.EndGame
	}{
		{
			name: "valid",
			fields: fields{
				Message: Message{TypeEndGame},
			},
			want: inbound.NewEndGame(),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := m.ToEvent(); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("EndGame.ToEvent() = %v, want %v", got, tt.want)
			}
		})
	}
}
