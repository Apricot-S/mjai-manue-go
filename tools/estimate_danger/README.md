...
- `<PATH/TO/FEATURES_FILE>`  
  Path to the extracted feature data file (gob format) generated by the `extract` command.
...
The `dump_tree` command loads a saved decision tree file and displays its structure in human-readable text format.

### Usage

```sh
go run ./tools/estimate_danger dump_tree <PATH/TO/TREE_FILE>
```

Arguments

- `<PATH/TO/TREE_FILE>`  
  Path to the decision tree file (gob format) generated by the `tree` command with the `-o` option.

### What It Does

- Loads a saved decision tree from the gob-encoded file
- Displays the complete tree structure with the same formatting as the `tree` command

### Output

Outputs the decision tree structure to standard output:

- Hierarchical tree structure with indentation
  - Node probabilities with confidence intervals and sample counts
  - Feature splits with true/false branches

The output format is identical to the final tree structure displayed by the `tree` command, but without the generation process information.

### Example of Usage

```sh
go run ./tools/estimate_danger dump_tree decision_tree.gob
```
...
The `dump_tree_json` command converts a saved decision tree file to JSON format for use in the AI system configuration.

### Usage

```sh
go run ./tools/estimate_danger dump_tree_json -o <OUTPUT_FILEPATH> <PATH/TO/TREE_FILE>
```
...
- Loads a saved decision tree from the gob-encoded file
- Converts the tree structure to JSON format
- Saves the JSON output to the specified file
...
- Nested structure representing the tree hierarchy
  - Node probabilities with confidence intervals and sample counts
  - Feature splits with positive/negative branches
...
```sh
go run ./tools/estimate_danger dump_tree_json -o danger_tree.all.json decision_tree.gob
```
