...
var supaiCriteria = []SupaiCriteriaEntry{
	...
	},
	{
		...
		Test: []Criterion{
			{"tsupai": false, "suji": true, "4<=n<=6": false}, // Omotesuji.
			{"tsupai": false, "suji": true, "4<=n<=6": true},  // Nakasuji.
	...
	},
	{
		...
		Test: []Criterion{
			...
			// same_type_in_prereach>=5 is too rare in the original data set.
...
func getNumberCriteria(baseCriterion Criterion) []Criterion {
	...
	for i := 1; i <= 5; i++ {
		...
		if i > 1 {
			...
			if value, ok := criterion[name]; ok && !value {
				continue
			}
			criterion[name] = true
		}
...
func buildInterestingCriteria() []Criterion {
	...
	for _, entry := range supaiCriteria {
		...
		criteria = append(criteria, entry.Test...)
	}

	for _, entry := range supaiCriteria {
		criteria = append(criteria, getNumberCriteria(entry.Base)...)
		for _, testCriterion := range entry.Test {
			criteria = append(criteria, getNumberCriteria(testCriterion)...)
		}
	}

	return slices.DeleteFunc(criteria, func(c Criterion) bool {
		return c == nil
	})
}
