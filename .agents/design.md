...
- `.agents/tools-porting.md`: 未移植の `tools/` の参照元、現行 schema、実装順、受け入れ条件。
...
- `tools/` は `dump_game_stats` / `dump_light_game_stats` / `postprocess_light_game_stats` / `print_game_stats` を現行 Go 実装へ移植済み。残る主対象は `estimate_danger`。
