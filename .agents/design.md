...
本設計書は、上記の既存資産を活用し、RiichiLab bridge、補助ラッパースクリプト、original-vs-port 検証基盤を段階的に足していく前提で進める。
...
- 今後、CoffeeScript 版 original と現行 Go port を同じ fixture / seed / 入力で実行し、protocol action を比較する開発者向け Framework を追加する。
- `log` や stderr trace は action 比較とは分け、必要な調査時だけ別モードで比較する。
...
8. **RiichiLab bridge**
   - RiichiLab 用の bridge を追加する。`cmd/` に Go アプリケーションを増やすか、`scripts/` に Python スクリプトを追加するかは実装前に再検討する。
   - WebSocket ↔ stdio JSON Lines の変換、bot token の付与、切断時の終了、`request_action` による出力タイミング調整を bridge 側へ閉じ込める。
   - `request_action` は domain/application へ転送しない。
   - `possible_actions` は入力にあっても信頼せず、合法手は State から算出する。

9. **`log` フィールド JSON 化ラッパー**
   - `mjai-manue` の出力を書き換え、文字列の `log` フィールドを解析ツールが扱いやすい JSON object へ変換する外部ラッパーを追加する。
   - 本体へ組み込むと余計な CLI オプションが増え、移植範囲からも外れるため、外部スクリプトとして実装する。
   - ラッパーを直接使わない場合でも、後処理や解析用の参考実装として利用できる形にする。
   - stdout は変換後の protocol output 専用、診断や変換不能な入力のエラーは stderr に出す。

10. **Original-vs-Port Testing Framework**
   - CoffeeScript 版 original と現行 Go port の出力差分を調査する開発者向け検証基盤を作成する。
   - 通常 CI へ無理に組み込まず、必要時に fixture / 牌譜 / seed を指定して再現確認できる形を優先する。
   - 比較単位は原則として protocol action とし、`log` や trace など説明出力の細部は目的に応じて別比較に分ける。
   - 実行に Node.js / CoffeeScript 版依存や追加データが必要な場合は optional dependency として明示し、通常の Go ビルド・テストから分離する。
