...
- `zzz_before_fix_bug_5r.txt` の先頭 mismatch は、立直宣言牌で通常 `5p` と赤 `5pr` の選択が分かれる局面だった。
- original では両候補の評価値が完全に同じになる。これは上記の赤5評価バグにより、赤5を切っても将来和了価値に赤5が残るためである。
- Go port では赤5を残す通常 `5p` 切りのほうが `expPt` は高いが、`avgRank` は赤 `5pr` 切りのほうが良く出ていた。
- 調査したところ、`hojuProb` / `myHoraProb` / `ryukyokuProb` / `otherHoraProb` は両候補で同一で、差分は自己和了時の点数分布だけだった。
- 自己和了時のツモ支払いでは `1/3` や `1/4` の score delta が出るため、相対点差が `3250` のような 100 点刻みでない値になる。
- `winProbsMap` は 100 点刻みの統計テーブルであり、original は存在しない相対点差キーでは統計を使わず、席順/点差によるフォールバックへ落ちる。
- Go port も original 互換として、統計テーブルに存在しない相対点差では補間せずフォールバックする。
- そのため、赤5を残して和了点が上がる候補でも、非 100 点刻みの相対点差が増えることで `avgRank` が直感と逆転する場合がある。
- これは赤5評価修正後に見えやすくなった original 互換の順位評価近似であり、今回は追加補間などの新規ロジックは入れない。
