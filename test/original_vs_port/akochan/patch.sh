...
# Change the connection destination from localhost to "server"
sed -i '
/TcpClient::TcpClient(const std::string &ip, const int port){/,/^}/{
    ...
    boost::asio::ip::tcp::resolver::results_type endpoints = resolver.resolve("server", std::to_string(port));\
