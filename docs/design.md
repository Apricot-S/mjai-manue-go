...
- `internal/application/` に Bot と入力への反応（`NoReaction` / `Action`）が実装されている。現状の action 判定は `round.State.LegalActions(selfID)` が空かどうかを参照する。`LegalActions` はまず自摸後・副露後など `pendingDiscard` が立つ局面の打牌候補列挙まで実装済みで、リーチ/和了/副露/見送り候補は今後追加する。Agent へ渡す観測は `round.ActionStateViewer` として、局面 view と合法手一覧の両方を含む。
...
現状実装では 5〜7 は `LegalActions(selfID)` を入口にしており、まず打牌候補だけを列挙する。副露・和了・見送りなど複数 actor の同時行動機会は、必要に応じて各 playerID の `LegalActions(playerID)` を問い合わせて判定する。
