...
- `cmd/mjai-manue/` は CLI を実装済み。`--name` / `--seed` / stdio / mjsonp TCP client の runtime 配線を持ち、起動時に `configs.LoadGameStats()` と `configs.LoadDangerTree()` を読み込んで deps validation 済みの `ManueAgent` を生成する。`internal/domain/ai/ManueAgent` は seed 注入済みで、`start_game` ごとに乱数生成器を seed 初期状態へリセットする。通常手番は打牌/立直候補を stats、危険度推定、和了推定、流局、他家和了、順位期待値へ接続して評価する。副露反応は `none` と鳴き後打牌候補を同じ評価経路で比較する。
...
ManueAgent.Decide
  ...
  │   │   └─ prefer tsumogiri over hand discard for the same exact tile
...
6. **`mjai-manue` 本体の追加（一部完了）**
   ...
   - 和了優先、リーチ後ツモ切り、打牌/立直候補、stats validation、`cmd/mjai-manue` からの stats / danger tree load、通常手番と副露反応の候補評価接続は実装済み。`manue_prob_dist.go`、score / rank / trace 系の純粋関数、和了推定に使う壁生成・試行・accumulator は残す。単なる field 代入 helper や未接続 wrapper は削除済みで、残りは `docs/manue-ai-porting-plan.md` に従って CoffeeScript 版との意味対応を詰める。
...
7. **CoffeeScript 版ロジックの再移植**
   ...
   - 合法手に同一牌の手出し discard とツモ切り discard が両方ある場合は、候補生成時点で手出し側を省き、ツモ切り側へ正規化する。CoffeeScript 版は候補 key を選び、出力時にツモ牌と `Pai.equal` で同一なら `tsumogiri=true` にしていたため、この正規化で現行 Go の action 差分を吸収する。赤牌差分は同一扱いしない。
   - 危険度推定、和了推定、流局、他家和了、順位期待値を candidate evaluation へ接続し、通常手番と副露反応の両方を CoffeeScript 版 `getMetricsInternal` 相当の評価値で選ぶ。副露反応は `none` と鳴き後打牌候補を同じ評価経路で比較する。評価語彙は `dealInProb`、`winProb`、`exhaustiveDrawProb`、`scoreDeltaDist` のように English/domain 寄りの名前を使う。
