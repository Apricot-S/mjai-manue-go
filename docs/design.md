...
- `cmd/mjai-manue/` は CLI と runtime 配線を持つ。`--name` / `--seed` / stdio / mjsonp TCP client、stats / danger tree の load、Agent 生成動線は整備済み。ただし `internal/domain/ai` の Manue 固有ロジックは途中実装に引っ張られた構造が残っているため、CoffeeScript 版の原仕様化と Go 移植仕様化を先に行ったうえで再実装する。
...
`ManueAgent` の再構築では、完成形と異なる独自評価を増やさない。通常手番、副露反応、危険度、和了推定、流局、他家和了、順位期待値は CoffeeScript 版 `getMetricsInternal` と同じ意味の評価値へ寄せる。既に `domain/game` にあるルール判定（合法手、向聴、役、点数、聴牌、和了形）は再実装しない。trace log の action key や表形式はオリジナル実装と揃え、内部構造は現行 Go のドメイン語彙へ寄せる。
