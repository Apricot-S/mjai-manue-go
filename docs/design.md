...
- `0`: 正常終了（例: stdio は `end_game` 受領後の EOF、mjsonp TCP client は `end_game` 受領後の正常クローズ）
...
- 再接続はしない。
- 切断時は即終了。
- CoffeeScript 版の TCP client 実装抜粋は `docs/tcp_client_game.coffee` に置く。Go 版は完全同一動作にはせず、I/O 安全性・終了コード・エラー返却を本設計書の方針に合わせる。
...
- CoffeeScript 版の `connect` / `close` / `error` に対応するログは Go 版にも反映する。ただし stdout は protocol output 専用なので、`connected` / `closed` / `tcp error: ...` 相当は stderr 側の log writer へ出す。
...
レビュー負荷を抑えるため、以下は1件ずつ独立した差分で実施する。完了済みの作業はこの一覧から外す。

1. **mjsonp TCP adapter の CoffeeScript 版由来仕様反映**
   - `docs/tcp_client_game.coffee` を参照するが、`hello` の `protocol` / `protocol_version` 検証は追加しない。他の mjai 互換サーバー実装でも使えるようにする。
   - CoffeeScript 版の socket lifecycle log（`connected` / `closed` / `tcp error: ...` 相当）を stderr 側 log writer へ反映する。stdout には出さない。
   - `error` message 受領時は response を送らず server error として終了する。Go 版では CoffeeScript 版の単純 close ではなく error を返す。
   - `end_game` は現行方針どおり response を送らず正常終了する。
   - 行バッファ、trace、`NoReaction` の同期応答 `none`、flush の現行方針は維持する。
   - 追加・変更する挙動は `internal/adapter/mjai/runtime` の単体テストで固定する。

2. **内部 API と非自明処理へのコメント追加**
   - doc comment は内部 API として必要な範囲に留め、不変条件・コーナーケース・移植仕様上の注意点を中心に追記する。
   - 例: `VisibleHand` は通常 5m/5p/5s と赤 5m/5p/5s を別 ID として扱うため、同じ通常 5 を4枚まで受け入れる一方、赤5は各色1枚までという制約を持つ。
   - 非自明な処理には短い解説コメントを追加する。ただし自明な getter や単純な代入にはコメントを増やさない。
