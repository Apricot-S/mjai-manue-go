...
- `internal/application/` に Bot と入力への反応（`NoReaction` / `Action`）が実装されている。現状の action 判定は `round.State.LegalActions(selfID)` が空かどうかを参照する。`LegalActions` は自摸後・副露後など `pendingDiscard` が立つ局面の打牌候補、自摸和了、ロン、ポン、大明槓、見送り候補まで実装済みで、チーなど残りの副露候補は今後追加する。Agent へ渡す観測は `round.ActionStateViewer` として、局面 view と合法手一覧の両方を含む。
...
現状実装では 5〜7 は `LegalActions(selfID)` を入口にしており、自摸後の打牌/リーチ/自摸和了、他家打牌後のロン/ポン/大明槓/見送りを列挙する。残りの副露候補など複数 actor の同時行動機会は、必要に応じて各 playerID の `LegalActions(playerID)` を問い合わせて判定する。
